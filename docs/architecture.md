...
The control host owns orchestration and native inference; a dedicated simulation worker owns gem5 execution. CHIA dispatch uses explicit resource labels. Local dispatch uses the same functions without pretending to be distributed execution.

The software workload is inference-only. Qwen2.5 0.5B Q5_K_M runs natively through a loopback llama.cpp server on the control host. The attention-kernel proxy executes in gem5 and does not run the Tutor model.
