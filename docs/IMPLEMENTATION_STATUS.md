...
| 7. Control host / gem5 worker (B) | IMPLEMENTED | Remote placement and synchronized project imports verified: control/software and hardware tasks reached their resource-labelled hosts. |
...
| 15. Combined deterministic execution (B) | IMPLEMENTED | A Qwen/llama.cpp plus gem5 candidate completed across the two-host CHIA cluster and produced a combined record. |
...
| E. Proxy calibration | PARTIAL | Exact GGUF inspection, native sweep, corrected mapping/dispatch and the 30-job repeated two-profile comparison are complete. Both compared profiles and the result/checksum record are preserved. Final KV-format, normalized-error and proxy-selection decisions remain TODO; equivalent native hardware ranking is unavailable. |

## Verification

- Current branch verification: **117 tests passed, 4 native-compiler cases skipped on the Windows checkout, and 2 scheduling tests deselected**. No failed tests. The skipped C cases run automatically where `gcc` is available.
...
- Server acceptance evidence: CHIA brought up one control host and one gem5 worker; resource placement and project imports passed on both hosts. Both Ollama script invocation styles completed; the warm run reached about 33.35 generated tokens/s and 2.19 seconds total latency. The simulation worker found Docker and the preinstalled `ghcr.io/gem5/devcontainer:v25-1` image.
- Full-loop server acceptance: the 2026-09-15 acceptance candidate `36decb...d054` completed in 167.48 seconds. Software completed in 5.46 seconds; hardware completed in 164.99 seconds; evaluation completed and the candidate entered the diagnostic Pareto frontier.
...
- 2026-09-15: two-host cluster startup and resource placement passed. Added the synchronized project root to control/worker `PYTHONPATH` after the worker correctly rejected the previously missing `src` import.
- 2026-09-15: server rerun verified imports on both hosts, both Ollama entrypoint forms, warm-model inference, Docker access and the installed gem5 image. The next gate was one combined candidate with a reviewed Q4 numerical tolerance.
...
- 2026-09-15: the first combined server attempt validated and ran software successfully, but the hardware subprocess exited with code 2 before simulation completed. Failure records now identify the controlled hardware stage and retain only byte-count/hash summaries; raw tool output remains local and omitted.
...
- 2026-09-15: the fourth acceptance attempt completed the first real distributed deterministic candidate end to end. The next gate was record/provenance review and model-digest pinning before the three-candidate campaign.
...
- 2026-09-17: implemented and executed the Issue #60 proxy-fidelity protocol without changing the original proxy dimensions: exact GGUF metadata, controlled native and gem5 context sweeps, gem5 sensitivity, normalized trend plot, Spearman context correlation and evidence report. Native hardware-rank correlation is explicitly unidentifiable on the available fixed native host.
- 2026-09-17: the first 128-token server simulation completed but exceeded the reviewed Q4 limits (`0.010727912` maximum absolute error and `0.000012264` MSE). The production loop remains fail-closed; the calibration path now retains finite raw errors, labels tolerance violations and continues the evidence sweep instead of discarding later cases.
- 2026-09-17: the original five-context proxy sweep followed the native Qwen context-growth trend closely, but architecture review confirmed the 4/2/32 proxy does not match Qwen's 14/2/64 attention shape.
- 2026-09-17: a temporary 14/2/64 pilot exposed incorrect grouped-query KV mapping and incomplete head dispatch. Correcting both reduced pilot maximum error by about 24 times and MSE by about 642 times relative to the broken pilot. Canonical implementation and the corrected full sweep were pending at that stage; see `docs/experiments/proxy-calibration-handoff.md`.
- 2026-09-18: implemented the canonical grouped-query and generic head-dispatch corrections, relative-error metrics, calibration-only candidate validation and a checkpointed three-trial comparison of corrected 4/2/32 versus Qwen-shaped 14/2/64 profiles. Native benchmark evidence reports F16 K/V while the proxy remains packed Q4. Distributed execution was the next gate.
- 2026-09-18: completed all 30 distributed comparison jobs. Both profiles had zero simulated-time variance and Spearman rho `1.0`; normalized-trend MAPE was `3.580%` for corrected 4/2/32 and `3.668%` for Qwen-shaped 14/2/64. The Qwen-shaped profile violated the old absolute Q4 tolerance at all contexts. Both choices and the checksummed experiment record are retained pending team review.
