...
| 11. Failures (C) | IMPLEMENTED | Candidate and request deadlines are separate. Transient availability failures have bounded audited retries; request timeouts are not retried because the single llama.cpp slot may still be processing the abandoned request. Failed records include sample, attempt and safe slot-state provenance. |
...
| E. Final inference evaluation | PARTIAL | Qwen2.5 0.5B Q5_K_M, GGUF hash and a 100-question custom mixed QA concept-scoring set are wired; a held-out set with reviewed provenance remains future work. |
| E. Software-only optimization | PARTIAL | The deterministic 3/9-candidate sweeps and Gemini-guided proposer are implemented for temperature and maximum output tokens. The 100-question × 2-repetition server campaign must be rerun with the audited timeout behavior before results are accepted. |
...
- Current local suite: **142 tests passed, 3 scheduling/environment tests skipped**. Contract validation also passes. A real llama.cpp smoke test and long software campaign remain server-side acceptance gates for this change.
...
- 2026-09-16: replaced the Ollama adapter with the pinned Qwen2.5 0.5B Instruct Q5_K_M llama.cpp service, added GGUF/build/context/slot preflight, evaluator-only references, deterministic concept coverage, schema-aware Gemini output, and final combined-run configuration. The local suite passes; one remote combined acceptance run remains.
