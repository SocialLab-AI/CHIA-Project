...
| E. Proxy calibration | PARTIAL | Exact GGUF inspection, native/original-proxy sweeps and sensitivity evidence exist. Grouped-query mapping, generic head dispatch, normalized errors and a checkpointed repeated comparison runner are implemented. The two-profile distributed comparison and final decision remain TODO; equivalent native hardware ranking is unavailable. |

## Verification

- Current branch verification: **117 tests passed, 2 native-compiler cases skipped on the Windows checkout, and 2 scheduling tests deselected**. No failed tests. The skipped C cases run automatically where `gcc` is available.
